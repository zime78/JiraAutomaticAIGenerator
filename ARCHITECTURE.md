...
> Jira 이슈 자동 분석 및 Claude AI 연동 macOS GUI 애플리케이션
...
┌─────────────────────────────────────────────────────────────┐
│                         UI Layer                             │
│    internal/ui/ (Fyne GUI, 상태관리, 컴포넌트)               │
└──────────────────────────┬──────────────────────────────────┘
                           │ 의존
┌──────────────────────────▼──────────────────────────────────┐
│                      UseCase Layer                           │
...
## 3-Channel Independent Architecture

```
┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
│   Channel 1     │  │   Channel 2     │  │   Channel 3     │
├─────────────────┤  ├─────────────────┤  ├─────────────────┤
│ URL 입력        │  │ URL 입력        │  │ URL 입력        │
│ 프로젝트 경로   │  │ 프로젝트 경로   │  │ 프로젝트 경로   │
│ 처리 버튼       │  │ 처리 버튼       │  │ 처리 버튼       │
│ 진행률          │  │ 진행률          │  │ 진행률          │
│ 대기 큐         │  │ 대기 큐         │  │ 대기 큐         │
│ 결과 탭         │  │ 결과 탭         │  │ 결과 탭         │
...
type ChannelState struct {
    ...
    ProgressBar, ResultText, AnalysisText    // 결과 위젯
    StatusLabel, QueueList, InnerTabs        // 상태 위젯
    CurrentDoc, CurrentMDPath                // 현재 상태
    CurrentAnalysisPath, CurrentPlanPath     // 분석 경로
}
```
...
[claude]
cli_path = claude
project_path_1 = /path/to/project1   # 채널 1 필수
project_path_2 = /path/to/project2   # 채널 2 필수
project_path_3 = /path/to/project3   # 채널 3 필수
enabled = true
```
